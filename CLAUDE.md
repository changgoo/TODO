...
- `SFIR/SFIR.json` — Structured schedule for the Star Formation/ISM Rendezvous (SFIR) seminar (Fall 2025 + Spring 2026); source of truth for email automation
- `SFIR/SFIR.md` — Human-readable markdown mirror of `SFIR/SFIR.json`
- `SFIR/sfir_email.py` — Sends seminar reminder emails via SendGrid; run with `--mode speaker|friday|dayof`; supports `--dry-run`
...
- Update `SFIR/SFIR.json` when speaker info (email, title, abstract) is confirmed
